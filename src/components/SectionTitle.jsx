  ...
  return (
    <div className={`mb-16 max-w-3xl ${alignmentClasses[align]} ${className}`}>
      ...
      <motion.h2 
        ...
        className="text-3xl md:text-4xl lg:text-5xl font-serif text-text leading-tight"
      >
        {title}
      </motion.h2>
      {align === 'center' && (
        <motion.div 
          ...
          className="h-1 bg-brand mx-auto mt-6 rounded-full"
        />
      )}
      {align === 'left' && (
        <motion.div 
          ...
          viewport={{ once: true }}
          transition={{ delay: 0.2, duration: 0.5 }}
          className="h-1 bg-brand mt-6 rounded-full"
        />
      )}
    </div>
  );
